...
public class Controller extends Node {
	...
	public synchronized void start() throws Exception {
		...
		StdOut.println("Controller initialised at (" + this.controllerPort + ")...");
		StdOut.println("Waiting for contact at controller...");
	...
	public void doDistanceVector() throws IOException {
		...
		StdOut.println("Finding shortest path between end users in network...\n");
		...
		for(int src : this.endUsers) {
			...
			for(int dst : otherEndUsers) {
				StdOut.println("Shortest path from " + src + nodeNames.get(src) + " to "+ dst + nodeNames.get(dst) + " is as follows:");
				...
				for(int node : path) {
					...
					if(!endUsers.contains(node)) {
						...
						i++;
						StdOut.println(node + nodeNames.get(node));	
	 ...
	 * Implemented using (https://codereview.stackexchange.com/questions/84717/shortest-path-using-breadth-first-search)
	 * 
	...
	public void printNetworkConnections() {
		...
		StdOut.println("Node Address           |   Connections" );
		StdOut.println("---------------------------------------------");
		...
		for (Entry<Integer, ArrayList<Integer>> entry : this.networkConnections.entrySet()) {
			...
			StdOut.println("" + nodeAddress + " " + nodeNames.get(nodeAddress) + "    |       ");
			
			for(Integer i : connections)
				StdOut.println("                                |       " + i + " " + nodeNames.get(i));
	...
	public void onReceipt(DatagramPacket packet) {
		try {
			...
			case 1:
				StdOut.println("Packet is from router seeking flow update...");
				//handleUpdateRequest();
				break;
			//If flag = 2, packet is from a node informing of connections
			case 2:
				StdOut.println("Packet is from node informing of connections...");
	...
	public void handleConnectionInform(DatagramPacket packet) {
		...
		StdOut.println("Handling connection inform packet...");
		...
		this.nodeCount++;
		StdOut.println("Node count = " + this.nodeCount);
		StdOut.println("Network connections updated...\n");
	}
