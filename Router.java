...
public class Router extends Node {
	...
	public synchronized void start() throws Exception {
		StdOut.println("Initialising routing map at router (" + this.routerPort + ")...");
		this.initialiseRoutingMap();
		
		StdOut.println("Filling in node names in map...");
		this.addNodeNames();
		
		StdOut.println("Printing routing map at router (" + this.routerPort + ")...\n");
	...
	public synchronized void informController() throws IOException {
		...
		socket.send(packet);
		StdOut.println("Controller informed of connections...");
	}
	...
	public void initialiseConnections() {
		...
		StdOut.println("Connections initialised...");
	}
	...
	public void printRoutingMap() {
		...
		StdOut.println("Destination   |    NextHop    |   HopCount" );
		StdOut.println("-------------------------------------------------------");
		...
		for(Entry<Integer, RoutingElementKey> entry : this.routingMap.entrySet()) {
		    ...
		    StdOut.println("" + this.nodeNames.get(destination) + "       |       " + this.nodeNames.get(nextHop) + "              |         " + hopCount);
		}
	}
	...
	public void continueTransmission(DatagramPacket packet) throws IOException, InterruptedException {
		...
		if(this.routingMap.get(content.getDestination()).nextDest != 0){
			StdOut.println("Router knows how to get to destination...");
	...
	public void processControllerUpdate(DatagramPacket packet) throws InterruptedException, IOException{
		...
		StdOut.println("Processing controller update...");
		...
		StdOut.println("Controller update processed successfully...");
		StdOut.println("Updating maps...\n");
		printRoutingMap();
	}
